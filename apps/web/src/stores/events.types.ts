...
// them. These four do. The walkthrough advances its "Create the account" step
// on 'created', because a step that asks the user to do something has to advance
// on the action completing, not on a "Next" click. The two demo reasons and
// 'csv-imported' each add or remove a batch of positions, fills and ledger rows
// in one call, so every derived surface in the app is stale at once — and
// neither the seeding hook nor the import hook knows any of their query keys.
// Cross-feature invalidation goes through this bus precisely so no feature has
// to import another's keys.
export type AccountChangeReason = 'created' | 'demo-seeded' | 'demo-removed' | 'csv-imported';
