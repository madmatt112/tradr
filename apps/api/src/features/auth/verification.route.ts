 ...
 *       token guessing is primarily throttled by the token's 256-bit entropy +
 *       24 h TTL). Atomically consumes the single-use verification token and
 *       marks the account email-verified in one transaction. Expired,
 *       already-consumed, and unrecognized tokens are indistinguishable: one
 *       generic 400 INVALID_OR_EXPIRED_TOKEN (REQ-5.3 — the frontend then
 *       offers the resend path). Consumption is THIS POST — fetching the
 *       emailed GET link never mutates, so prefetchers and link scanners burn
 *       nothing (D6, REQ-4.8). Fully functional when the instance has no email
 *       configured (D12 — consuming an existing token sends nothing).
 *     tags: [Auth]
 *     requestBody:
 *       required: true
 *       content:
 ...
 *     description: >
 *       Authenticated (session cookie) — only the account owner can trigger a
 *       resend, so account enumeration is impossible by construction (D11).
 *       Rate limited per authenticated userId (3 / 1 h — per-account equals
 *       per-target, REQ-3.8's bound). No request body. Issues a fresh
 *       single-use verification token (24 h TTL, newest-wins over prior
 *       tokens) and dispatches the verification email WITHOUT awaiting
 *       delivery (REQ-2.7). Already-verified accounts get 409 ALREADY_VERIFIED
 *       (an authed self-fact); an instance with no email configured gets 409
 *       EMAIL_NOT_CONFIGURED — the two codes are distinct because the UI keys
 *       on code (SF-2, D12).
 *     tags: [Auth]
 *     responses:
