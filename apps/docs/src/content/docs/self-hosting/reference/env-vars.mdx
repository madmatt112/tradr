...
Tradr reads **74 environment variables**, of which **3 are required** —
...
Of the 74 variables, **46 ship unset** — either commented out in the
...
| `CSV_IMPORT_NGINX_PROXY_TIMEOUT` | `300s` | nginx proxy read timeout for the dedicated /api/csv-import/ AND /api/account-data/ routes (both reuse this one value). A large within-cap synchronous CSV import, account export or account import can exceed nginx's default 60s; this raises the timeout for those routes only (others unchanged). Keep CSV_IMPORT_CLAIM_TIMEOUT_SECONDS (api) ≥ 2 × this value — the two live in different containers and are hand-synced. |
| `MAX_UPLOAD_SIZE` | `20m` | nginx client_max_body_size for uploads. Must accommodate the largest upload a single request can carry, or large uploads 413. OPERATOR CONSTRAINT: keep MAX_UPLOAD_SIZE ≥ CSV_IMPORT_MAX_FILE_BYTES (below), or oversized CSV imports are rejected by nginx before the api can size them. OPERATOR CONSTRAINT: also keep MAX_UPLOAD_SIZE ≥ the position-screenshot request (one image, at most 4,504,096 bytes), or screenshot uploads are rejected by nginx before the api can size them. It ALSO caps uploaded account-import archives (/api/account-data/import*): an archive larger than this is refused by nginx with 413 before the api can size it, and the app tells the user the operator can raise this value. |
...
| Variable | Default | Notes |
| --- | --- | --- |
| `STRIPE_SECRET_KEY` | Optional | ALL optional. When unconfigured the billing feature is ABSENT (not broken): no purchase UI. Both STRIPE_SECRET_KEY and STRIPE_WEBHOOK_SECRET are required to enable the purchase + webhook path. Stripe secret key (server-side). Required for purchases. Optional. |
...
| `POSTHOG_PERSONAL_API_KEY` | Optional | Opt-in account-deletion person purge. Set BOTH to delete the PostHog person on account deletion (best-effort, non-blocking): a personal API key with the person:write scope, and the numeric project id. Absent ⇒ the person record is left in place. Distinct from POSTHOG_API_KEY (the ingestion project key above); the REST API lives on the app host (e.g. us.posthog.com), not the ingestion one. |
...
| `POSTHOG_PUBLIC_FEEDBACK_SURVEY` | Optional | Feedback-tab survey ids, "&lt;surveyId>:&lt;ratingQuestionId>:&lt;textQuestionId>" (the three ids from a PostHog survey created with the API presentation). Optional and needs POSTHOG_PUBLIC_KEY set; absent, empty, or malformed ⇒ no feedback tab renders and the SDK's surveys UI is never loaded. See the privacy posture above for what the form sends. |

## Hosted platform (api container)

| Variable | Default | Notes |
| --- | --- | --- |
| `REDIS_URL` | Optional | ALL optional — SELF-HOSTERS LEAVE EVERYTHING BELOW UNSET. Each capability is a no-op when its config is absent/empty: with none configured the stack behaves EXACTLY as today (uploaded images inline base64-in-JSONB, process-local rate limiter, same-origin SameSite=Lax cookies, migrations over DATABASE_URL, prepared statements on). These are only needed to run the managed offering. An empty value is treated as unset. PRIVACY POSTURE: object storage and split-origin add NO third-party trackers — uploaded images move from one operator-controlled store (Postgres) to another (the operator's own bucket), never to a shared/third-party surface. Object- storage credentials and REDIS_URL are SECRETS: server-side env only, never emitted to the frontend /config.js seam. Centralized rate-limit store (REQ-7). Set to a redis:// URL to share one rate limit across N API containers; unset ⇒ process-local in-memory limiter. Optional. |
...
| `OBJECT_STORAGE_ENDPOINT` | Optional | Object storage for uploaded images (REQ-2). All FOUR of ENDPOINT + BUCKET + ACCESS_KEY_ID + SECRET_ACCESS_KEY are required to enable it; partial/unset ⇒ images stay inline base64-in-JSONB as today. S3-compatible (Cloudflare R2 / AWS S3 / MinIO). All optional. |
