...
Tradr's schema is **37 tables**, created by **37 migrations** that run
automatically when the api boots. This page is generated from the Drizzle snapshot
for the latest schema-changing migration (`0036_account_deletion_tables`), so it describes the schema the migrations
actually produce.
...
| `purge_outcome` | `text` | not null, default `'pending'` |
