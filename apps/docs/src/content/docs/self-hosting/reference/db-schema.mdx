...
Tradr's schema is **35 tables**, created by **36 migrations** that run
automatically when the api boots. This page is generated from the Drizzle snapshot
for the latest schema-changing migration (`0035_ledger_cash_movements`), so it describes the schema the migrations
actually produce.
