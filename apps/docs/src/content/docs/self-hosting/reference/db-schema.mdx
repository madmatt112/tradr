...
Tradr's schema is **34 tables**, created by **34 migrations** that run
automatically when the api boots. This page is generated from the Drizzle snapshot
for the latest schema-changing migration (`0033_tags`), so it describes the schema the migrations
actually produce.
