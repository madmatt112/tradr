...
whatever you wrote in your notes. It may also hold **API keys** you have pasted
in, which can spend money.
...
Keys you paste in are encrypted at rest with **AES-256-GCM**,
...
<SelfHosted>
  There is no lockout after N failed attempts, and no CAPTCHA. For an instance with a handful of
  known users that is a reasonable trade; if yours is exposed to the internet with open
  registration, put an authenticating proxy in front.
</SelfHosted>
